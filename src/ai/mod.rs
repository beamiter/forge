...
fn validate_client_values(model: &str, base_url: &str, max_tokens: u32) -> Result<(), AiError> {
    if model.trim().is_empty()
        ...
        || crate::review_input::contains_visual_spoof(model)
    {
        return Err(AiError::InvalidConfiguration(format!(
    ...
    if base_url.len() > MAX_BASE_URL_BYTES
        ...
        || crate::review_input::contains_visual_spoof(base_url)
    {
        return Err(AiError::InvalidConfiguration(
            format!(
...
fn expand_api_key_path(raw: &str) -> Result<PathBuf, AiError> {
    if raw.len() > MAX_API_KEY_PATH_BYTES
        ...
        || crate::review_input::contains_visual_spoof(raw)
    {
...
fn open_api_key_file(path: &Path) -> io::Result<fs::File> {
    ...
    if (metadata.uid() != effective_uid && metadata.uid() != 0)
        || (mode & 0o022 != 0 && mode & nix::libc::S_ISVTX == 0)
    {
...
fn trim_for_log(text: &str, max_bytes: usize) -> String {
    ...
    for ch in text.chars() {
        let visible = if ch.is_control() || crate::review_input::is_visual_spoof_character(ch) {
            '\u{fffd}'
        } else {
            ch
        };
...
mod tests {
    ...
    fn one_omission_notice() -> String {
        format!(
            "1 older conversation turn(s) were omitted by {}'s request safety budget. \
             Do not assume access to them.",
            crate::identity::get().app_name
        )
    }
    ...
    fn live_request_history_keeps_recent_complete_bounded_context() {
        ...
        assert_eq!(
            system,
            format!(
                "system\n\n22 older conversation turn(s) were omitted by {}'s request safety \
    ...
    fn cancellable_wait_kills_and_reaps_a_real_child() {
        ...
        let probe = unsafe { nix::libc::kill(pid, 0) };
        assert_eq!(probe, -1);
        assert_eq!(
