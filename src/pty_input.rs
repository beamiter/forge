...
impl AdmittedInput {
    ...
    /// Keep the prompt fail-closed until the next authoritative PromptEnd;
    /// otherwise a split opener can leave the shell inside a paste frame while
    /// Forge still advertises the editor as empty.
...
pub(crate) fn admitted_input(data: &[u8], initially_in_frame: bool) -> AdmittedInput {
...
pub fn classify_paste(text: &str) -> PasteRisk {
    ...
    let had_visual_spoofing = crate::review_input::contains_noncontrol_visual_spoof(&body);
...
mod tests {
    ...
    fn classify_matches_the_confirmation_policy() {
        ...
        assert!(
            ...
            "structural newlines are covered by the multiline risk, not Unicode spoofing"
        );
