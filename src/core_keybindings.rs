...
//!
//! anvil..4 each grew their own chord handling (GTK `parse_key_combo` /
//! `key_combo_to_string` in anvil/4, string-level `normalize_binding` in
...
//!   order anvil/4 already print and the storage order ember/3 already
...
impl NamedKey {
    ...
    /// as "Enter" — the family UI convention (ember's prettify, anvil/4
    /// display).
    ...
    /// storage form keeps `return` (ember/3 store `ctrl+shift+return`)
...
/// - `+` is also the separator, so a literal plus key uses the anvil/4
...
pub fn parse(s: &str) -> Result<Chord, ParseError> {
    ...
    if s.chars().any(char::is_control) || crate::review_input::contains_visual_spoof(s) {
    ...
    // "ctrl+") is a malformed chord, not a plus key — the historical anvil/4
...
fn parse_key_token(token: &str) -> Result<KeySym, ParseError> {
    ...
    let sym = match lower.as_str() {
        ...
        // X11-style symbol names (anvil/4 accept these via GTK's
...
/// forge but "TerminalSplitVertical" in ember/3, yet it is the same
...
///
/// - `ctrl+shift+a` — hard family conflict: SelectAllBlocks in anvil/4
///   vs AgentToggle in ember/3. No neutral meaning exists.
