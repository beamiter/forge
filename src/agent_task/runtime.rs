...
impl AgentRuntimeManager {
    ...
    pub fn start_codex(
        ...
        let worker = std::thread::Builder::new()
            .name(format!("forge-codex-prepare-{task_id}-{generation}"))
            .spawn(move || {
    ...
    fn task_manager() -> (TaskManager, TaskId) {
        ...
        let task_id = tasks
            .create(super::super::NewTask {
                ...
                provider: AgentProvider::Codex,
                repo_root: format!("/tmp/forge-runtime-test-repository-{token}").into(),
                worktree_path: format!("/tmp/forge-runtime-test-worktree-{token}").into(),
                branch: "forge/async-native-preparation".into(),
