...
不会改变失败/成功带来的状态量或姿态强度；seasoned 面对一次大的真人恢复仍保持
完整庆祝，只把「终于。」收成一句「嗯。」。最近工作日集合有 64 日排序窗口和
...
在键入、守望、反应、alternate screen、Static 模式或完整姿态放不下时，这次侧目
直接丢弃而不排队，因此切焦点后不会补演过期失败。
