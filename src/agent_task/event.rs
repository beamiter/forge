...
/// Maximum provider-owned resume identity retained by Forge.
pub const PROVIDER_SESSION_ID_MAX_BYTES: usize = 512;

static NEXT_AGENT_EVENT_EPOCH: AtomicU64 = AtomicU64::new(1);

/// Forge-local identity for one native Agent session.
///
...
/// for routing events back to the matching Forge task incarnation.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct NativeAgentSessionId(String);

impl NativeAgentSessionId {
    /// Generate an Forge-owned identity. Provider adapters must not substitute
    /// their wire/session identifiers here.
    pub fn new() -> Self {
        Self(Uuid::new_v4().to_string())
    }

    /// Parse a previously generated Forge identity at an internal boundary.
...
/// Forge-local identity for one logical Agent turn.
...
/// Forge-local claim token for one approval or permission request.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
