...
`UiState` coordinates tabs, panes, sidebars, search, configuration reloads, and actions. The file sidebar uses GTK4's supported model-view stack: `gio::ListStore`, `TreeListModel`, `ListView`, and `TreeExpander`. Directory scans run on named worker threads; generation checks prevent stale results from repopulating a changed root. Listing and file operations dispatch through `ui::remote_fs`, so the same tree browses the local disk or any configured ssh/docker remote host: a fixed POSIX sh probe script runs on the far side through the system `ssh`/`docker` binaries (script on stdin, single-quote-escaped positional parameters, bounded output, hard timeouts) and its NUL-separated listing is parsed back. A header selector switches location, a right-click menu offers New File/Folder, Rename, Delete, Copy, Cut, Paste and Refresh on both backends (paste is same-location only), and a remote tab that reports its cwd through OSC 7 pulls the tree onto its host.
