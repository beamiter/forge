...
workflow 是 `~/.config/forge/workflows/` 下的 `.toml` / `.yaml` / `.yml` 「保存的带参命令」，四个 jterm 终端共用同一份格式与同一个加载器（`jterm_core::workflows`），因此同一个文件在 anvil / ember / forge / frost 里含义相同。**没有声明 `default` 的参数不能留空**：留空或只填空白时插入被拒绝并报 `missing values: <名字>`，参数对话框在按 Insert 之前就用 “Still needs a value: …” 列出仍然空着的行——`kill -9 {pid}` 不会再插入 `kill -9 `。声明了 `default` 的参数（包括 `default = ""`）仍可渲染成空值：**「空值有没有意义」由文件说了算**，清空一个有默认值的字段是有意的空值，清空一个没有默认值的字段是缺值。模板支持 `{name}` 与 `{{name}}` 两种占位符，占位符名两侧的空格会被忽略（`{{ service }}` 等同于 `{{service}}`），`{{ }}` 是字面花括号转义且零参数 workflow 也走同一条渲染路径，未闭合的 `{{` 原样保留。字段类型必须正确：`default = 3000` 这类忘了加引号的值、tags 里的非字符串元素、缺 `name` 的 `[[args]]` 都会连带整份文件被拒绝，并留下 `workflows: skipping <路径>: <原因>` 日志（超限、符号链接、非 UTF-8 同理）。用户目录优先于 `FORGE_WORKFLOW_DIR`、已安装示例和源码树示例，同名先到先得；用户配置目录解析不到绝对路径时该层直接跳过，绝不相对当前工作目录解析。
