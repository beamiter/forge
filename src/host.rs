...
fn helper_command_for(
    ...
    // Resolve inside the host namespace, but strip empty/relative PATH entries
    // before changing to an untrusted project directory.
    let mut command = Command::new(flatpak_spawn_program());
    command.args(["--host", "--watch-bus"]);
    if let Some(cwd) = cwd {
        command.arg(format!("--directory={}", cwd.display()));
        ...
        assert_eq!(
            ...
            vec![
                ...
                format!(
                    ...
                    jterm_core::identity::get().app_version
                ),
