        ...
        let update = self.run(
            git_command_os(
                ...
                [
                    ...
                    OsString::from("-m"),
                    OsString::from("Forge clean worktree retirement"),
...
fn validate_branch_text(value: &str) -> Result<(), WorktreeError> {
    ...
    {
        return Err(WorktreeError::InvalidInput(
            "branch is outside Forge's strict safe ref grammar".to_string(),
        ));
    }
    Ok(())
}
...
fn unique_recovery_ref() -> String {
    let (nanos, sequence) = unique_numbers();
    format!("refs/forge/archive/{nanos:x}-{sequence:x}")
}
...
fn is_system_owner(uid: u32) -> bool {
    ...
    // only when Forge itself is not running as the same uid; the executable and
...
fn run_command_with_timeout(
    ...
    #[cfg(unix)]
    {
        ...
        // an abrupt Forge process exit after its worker thread is detached.
        unsafe {
            command.pre_exec(|| {
...
mod tests {
    ...
    impl TestDirectory {
        fn new(label: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "forge-worktree-{label}-{}-{nanos:x}-{sequence:x}",
                std::process::id()
            ));
    ...
    fn task_and_branch_validation_rejects_traversal_and_options() {
        ...
        assert!(validate_branch_text("forge/task-42").is_ok());
    }
