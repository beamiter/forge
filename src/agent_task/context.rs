...
/// while Forge still labels the attached command/context as complete.
...
pub const UNKNOWN_EXIT_STATUS_NOTE: &str =
    "[Forge context: shell reported no exit status; -1 is a compatibility sentinel.]\n";
...
/// `CompletedCommandRecord` has been evicted.
///
...
mod tests {
    ...
    fn context() -> SemanticCommandContext {
        SemanticCommandContext {
            ...
            cwd: Some("/workspace/forge".to_string()),
            cwd_after: Some("/workspace/forge".to_string()),
            exit_code: Some(101),
    ...
    fn conversion_preserves_prompt_fields_without_mutating_domain_provenance() {
        ...
        assert_eq!(block.cwd.as_deref(), Some("/workspace/forge"));
        ...
        assert_eq!(semantic.cwd_after.as_deref(), Some("/workspace/forge"));
    }
