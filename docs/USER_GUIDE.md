...
- alt-screen 程序沿用运行中命令的网格：卡片只去掉装饰，边距与内边距不变，因此进入/离开
  alt-screen 不会让程序收到 SIGWINCH（codex 的 `Ctrl+T` 浮层不再触发整段记录重放）。鼠标
  程序的滚轮上报按滚动量累计为整格（触控板像素增量按 10 像素一格换算，单个事件最多 10 格），
  并使用程序请求的编码（SGR/urxvt/UTF-8/旧式）。程序未关闭鼠标上报就退出时，命令结束与下一个
  提示符会清除鼠标状态。
...
- 在块内拖选文本后键盘焦点会留在那张卡片上，Block 快捷键仍然全部可用：方向键、
  ...
  块内过滤输入框和弹出菜单内的按键归它们自己。命令（例如 claude、codex、kimi）运行期间，
  这个把焦点带回来的按键也会照常交给程序（经实时终端自己的输入法，不会丢拼音首字母），
  没有被 Block 占用的 `Ctrl`/`Alt` 组合键同样会送达程序；停在空闲提示符时它只负责交还焦点。
  运行中的 `Ctrl+C`/`Ctrl+D` 兜底在焦点位于过滤框、历史面板搜索框等文本输入框时不再生效，
  这些键归输入框自己（复制等）。
- 命令运行期间仍有选中块时，`Up/Down`/`Delete` 让给程序（顺带清除选择），claude 的权限菜单
  可以直接用方向键操作；`Enter` 仍被选择占用并响铃，但同时清除选择，下一次 `Enter` 就交给
  程序；`Escape` 仍只取消选择，不会误中断 Agent。点击卡片标题栏上的按钮（复制输出、重跑、
  折叠等）只执行按钮本身，不再顺带选中卡片。
- `Ctrl+单击` 与右键菜单的 **Open Link** 同时识别纯文本 URL 与 OSC 8 超链接（claude 把
  “Security guide”这类带标签的链接写成 OSC 8），只打开 http/https 目标；悬停在 OSC 8 链接上
  时提示框显示真实目标。
- 把文件或文件夹拖进 pane 会插入经过 shell 引号处理的路径（末尾一个空格，不回车）。任何
  存在的本地路径都可以，不再只限图片；Block pane 以粘贴方式发送，程序开启了括号粘贴
  （DECSET 2004）时带粘贴边界，与 `Ctrl+Shift+V` 一致。含控制字符或双向混淆字符的路径仍会被
  拒绝（“Drop rejected: …”）。
- 完成块同时受 `max_visible_blocks` 和每 pane 128 MiB 估算内存预算约束；预算包含 ANSI 原文、重复显示副本、VTE/控件与图片，超限时从最旧块开始淘汰，最新块始终保留。每个完成 VTE 最多保留 1,048,576 个 cells（最多 4096 列），因此极端长输出只在界面中保留有界终端窗口；这一几何裁剪不会再额外影响复制/导出中已捕获的文本（最多 8 MiB）。命令结束时，带纵向光标移动或滚动区域的输出（codex 这类内联 TUI、`top`、多行进度条）按子进程最后一次收到的终端尺寸重放：内联 TUI 插到视口上方的历史完整保留在卡片与复制/查找文本中，原地重绘的循环收敛为最后一帧。捕获超过 8 MiB 丢掉开头、或重放保留的历史超出约 200 万个 cells 时，卡片标题栏显示暗色的 “Earlier output not retained”；普通行输出超出文本重放预算（约 100 万个 cells）而丢掉结尾时显示 “Output text truncated”，两者同时发生时显示 “Output only partly retained”。这条提示随块数据保存，撤销、恢复和重新加载历史后仍然显示。Block Kitty 图片每块最多 64 张；Unified 图片使用同样的 16 MiB/64-placement 上限，超出可见网格的几何会明确拒绝而非静默缩放。

命令运行中或 alt-screen TUI 活跃时，Enter 和应用所需按键继续发送给前台进程，不会误触发旧块回填。

命令运行超过约 2 秒后，跟随底部时 pane 右上角出现一枚紧凑的运行胶囊：`▶ 命令 用时`，
计时每秒推进，并带一键 Stop。实时卡片的顶部一旦到了胶囊下面（codex、kimi 这类内联 Agent
把卡片长到整页高），胶囊就隐去，不会盖住程序以为自己拥有的前几行；这时用实时终端里的
`Ctrl+C` 中断。滚动到历史中时整宽运行状态条立即出现（此时实时卡片已不可见），alt-screen
程序活跃时让位；最小化后状态条缩成右侧的一个按钮。瞬时命令不会让它闪现。拖动滚动条、
查找跳转、书签或面板跳转离开底部哪怕不到一张卡片高，也会停止跟随，不会被下一帧输出拉回
底部。Stop、跳到最新与卡片标题栏上的按钮被点击后不抢键盘焦点：之后的按键仍然交给程序，
`Enter`/空格不会再按一次 Stop。
