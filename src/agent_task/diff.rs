...
impl AgentDiffPanel {
    ...
    fn request_inner(&mut self, cwd: PathBuf, base: String) -> Result<(), DiffRequestError> {
        ...
        let spawn = std::thread::Builder::new()
            .name("forge-agent-git-diff".to_string())
            .spawn(move || {
...
fn is_trusted_system_owner(uid: u32) -> bool {
    ...
    // only when Forge itself is not running as the same uid; otherwise an
...
fn run_command_with_timeout(
    ...
    #[cfg(unix)]
    {
        ...
        // This read-only helper must not survive an abrupt Forge exit either.
        unsafe {
            command.pre_exec(|| {
...
mod tests {
    ...
    fn deadline_converges_when_exited_child_leaves_a_pipe_holding_descendant() {
        const HELPER_ENV: &str = "FORGE_DIFF_PIPE_HOLDER_HELPER";
    ...
    fn pipe_holder_helper_process() {
        if std::env::var_os("FORGE_DIFF_PIPE_HOLDER_HELPER").is_none() {
            return;
        }
    ...
    fn immutable_task_base_keeps_committed_agent_changes_visible() {
        ...
        let repository = TempRepo(
            std::env::temp_dir().join(format!("forge-task-diff-base-{}", uuid::Uuid::new_v4())),
        );
