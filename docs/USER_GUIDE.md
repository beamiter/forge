...
减，心情向能量/信心/压力合成的目标缓动。多个 Block pane 共享同一颗心灵和同
一个时钟，身体数量不会加快生命节奏。该模拟只存在于内存，落盘仍只发生在命令
生命周期事件。
...
临时试用可设 `FORGE_ASCII_ORGANISM_MOTION`。驱动本身也克制：帧驱动是每秒至多
十次的 glib 定时器（不强制 GTK 帧时钟满速运转），整窗安静一分钟进入静息后
降为每秒一次的心跳，被冷落的终端几乎不消耗 CPU；有任何输入或命令，下一拍
（至多一秒）即恢复全速。
...
游走占比提高。
行走有交替步态帧，观察输出越密集甩尾越快，庆祝会眨星、push 后守着尾巴打盹。
同一姿态组内的所有动画帧共享包围盒，空间身体的 fail-closed 尺寸判定不会因
动画帧抖动；错误/庆祝等反应姿态保持规范形态，不被身体语言覆盖。

空间身体不再瞬移：姿态切换时它以整格步进走过去（剩余距离的四分之一每帧、
至少一格，跨全屏约一秒，途中播放步态帧）。键入撤退与 alternate screen 的
立即隐藏语义不变——被隐藏后再出现是直接贴位，它从不在看不见的地方走路。
命令运行时的守望姿态也不再钉死右上角，而是贴着输出增长边缘上方一两行、
随光标下移一路跟下来（只取行号这一内容无关几何量），输出越深它蹲得越低。
...
累计三次以上后形成基线，之后的成功构建若比平时慢一倍以上或快一倍以上（且
