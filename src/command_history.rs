...
fn validate_history_path(path: &Path) -> io::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        if bytes.len() > MAX_HISTORY_PATH_BYTES
            ...
            || path
                .to_str()
                .is_some_and(crate::review_input::contains_visual_spoof)
        {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
...
fn validate_record_fields(command: &str, cwd: Option<&str>) -> io::Result<()> {
    ...
    if let Some(cwd) = cwd {
        ...
        if cwd.chars().any(char::is_control) || crate::review_input::contains_visual_spoof(cwd) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
...
mod tests {
    ...
    fn fifo_history_is_rejected_without_blocking() {
        ...
        assert_eq!(unsafe { nix::libc::mkfifo(path_bytes.as_ptr(), 0o600) }, 0);
