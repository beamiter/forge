...
// release audited by Forge; a newer install falls back to the terminal path
// instead of silently gaining new provider authority.
const SUPPORTED_CODEX_USER_AGENT_PREFIX: &str = "forge/0.147.0 ";
...
/// private in the worker and is addressed only through Forge's `ApprovalId`.
...
pub struct CodexAppServerViewSnapshot {
    ...
    /// Stable Forge-local identity for the current/latest flat projection.
...
impl AgentDriver for CodexAppServerDriver {
    ...
    fn start(&mut self, request: AgentStartRequest) -> Result<(), AgentDriverError> {
        ...
        let worker = thread::Builder::new()
            .name("forge-codex-app-server".into())
            .spawn(move || {
...
impl CgroupContainment {
    ...
    fn kill_all_and_wait_empty(&mut self) -> Result<(), WorkerFailure> {
        ...
        // Forge is abruptly killed, covering daemonized setsid descendants.
...
impl CgroupGuardian {
    fn spawn(kill: &File) -> Result<Self, WorkerFailure> {
        ...
        command
            .arg("-c")
            .arg("while IFS= read -r forge_guard_line; do :; done; printf '1\\n'")
            .stdin(Stdio::piped())
...
fn run_worker(context: WorkerContext) -> CodexAppServerExitReport {
    ...
    let scope_unit = format!("forge-codex-{}.scope", Uuid::new_v4());
    ...
    while terminal.is_none() {
        ...
        if terminal.is_none() && reader.eof {
            terminal = Some(TerminalIntent::failed(WorkerFailure::protocol(
                "app-server stdout closed before Forge ended the native session",
            )));
        }
        ...
        if terminal.is_none() {
            match process.leader_has_exited() {
                Ok(true) => {
                    let detail =
                        "Codex app-server exited before Forge ended the native session".to_string();
...
impl ProtocolMachine {
        ...
        match pending {
            PendingClientRequest::Initialize => {
                ...
                if codex_home != self.expected_codex_home {
                    return Err(WorkerFailure::protocol(
                        "app-server did not use Forge's isolated Codex home",
                    ));
                }
    ...
    fn queue_thread_start(&mut self, writes: &mut WireWriteQueue) -> Result<(), WorkerFailure> {
        ...
        self.queue_request(
            ...
            json!({
                ...
                // environment. Native tasks are confined to Forge's pinned
    ...
    fn handle_server_request(
        ...
        match method {
            ...
            _ => {
                writes.enqueue(&json!({
                    "id": rpc_id.value(),
                    "error": { "code": -32601, "message": "method not supported by Forge" }
                }))?;
            }
        }
        Ok(None)
    ...
    fn handle_notification(
        ...
        match method {
                ...
                return match status.as_str() {
                    ...
                    "interrupted" => {
                        ...
                        Ok(Some(TerminalIntent::failed(WorkerFailure::provider(
                            "Codex turn was interrupted without an Forge cancellation",
                        ))))
                    }
                    "failed" => {
                        self.clear_terminal_turn();
            ...
            // Stable notifications not needed for Forge's current view are
        ...
        match command {
            ...
            AgentCommand::DecideApproval { id, decision } => {
                ...
                if matches!(&decision, ApprovalDecision::Approve) {
                    return Err(WorkerFailure::protocol(
                        "native approval is disabled because accepted actions cannot yet be bound to Forge's workspace capability",
                    ));
                }
                self.approvals.remove(&id);
                ...
                // Deliberately use only the stable one-shot decisions. Forge
...
/// Prove that the provider resolved configuration only from Forge's private
...
fn attest_native_codex_config(
    ...
    for layer in layers {
        ...
        match kind {
            ...
            "user" if !user_seen => {
                ...
                {
                    return Err(WorkerFailure::protocol(
                        "native Codex user config is not Forge's private empty config",
                    ));
                }
            }
    ...
    for feature in NATIVE_CODEX_DISABLED_FEATURES {
        ...
        if origin != Some("sessionFlags") {
            return Err(WorkerFailure::protocol(format!(
                "native Codex feature {feature} did not originate from Forge's flags"
            )));
        }
    }
    ...
    if web_search_origin != Some("sessionFlags") {
        return Err(WorkerFailure::protocol(
            "native Codex web-search policy did not originate from Forge's flags",
        ));
    }
    for field in [
        "allow_login_shell",
        ...
        if config_origin_type(origins, field) != Some("sessionFlags") {
            return Err(WorkerFailure::protocol(format!(
                "native Codex policy {field} did not originate from Forge's flags"
            )));
        }
    }
    for name in expected_tool_environment.keys() {
        ...
        if config_origin_type(origins, &field) != Some("sessionFlags") {
            return Err(WorkerFailure::protocol(format!(
                "native Codex tool environment {name} did not originate from Forge's flags"
            )));
        }
    }
    Ok(())
...
mod tests {
    ...
    fn integer_rpc_ids_complete_versioned_handshake_and_preserve_cwd_root_split() {
        ...
        machine
            .handle_message(
                json!({"id": 1, "result": {"userAgent": "forge/0.147.0 (test)", "codexHome": "/private/codex"}}),
                &mut writes,
                false,
            )
            .unwrap();
    ...
    fn native_handshake_rejects_codex_home_mismatch_and_token_refresh() {
        ...
        assert!(mismatch_machine
            .handle_message(
                json!({"id": 1, "result": {"userAgent": "forge/0.147.0 (test)", "codexHome": "/home/user/.codex"}}),
                &mut writes,
                false,
            )
            .is_err());
        ...
        assert!(version_machine
            .handle_message(
                json!({"id": 1, "result": {"userAgent": "forge/0.148.0 (test)", "codexHome": "/private/codex"}}),
                &mut writes,
                false,
            )
            .is_err());
    ...
    fn server_request_resolved_removes_matching_approval() {
        ...
        // The server can resolve after Forge's click was already queued.
        machine
            .handle_message(
                json!({
                    "method": "serverRequest/resolved",
...
"#,
    ...
    fn cgroup_guardian_does_not_depend_on_a_single_digit_fd() {
        let path =
            std::env::temp_dir().join(format!("forge-cgroup-guardian-test-{}", Uuid::new_v4()));
    ...
    fn tool_environment_keeps_safe_toolchains_and_drops_repo_and_proxy_state() {
        ...
        let root = std::env::temp_dir().join(format!("forge-tool-env-{}", Uuid::new_v4()));
    ...
    fn guardian_write_failure_is_not_empty_without_events_evidence() {
        ...
        let path = std::env::temp_dir().join(format!(
            "forge-cgroup-events-evidence-test-{}",
            Uuid::new_v4()
        ));
    ...
    fn cgroup_scope_kills_term_ignoring_setsid_descendant_before_reap() {
        ...
        let scope_unit = format!("forge-codex-test-{}.scope", Uuid::new_v4());
        let mut command = Command::new(SYSTEMD_RUN);
        command
            .arg("--user")
            .arg("--scope")
